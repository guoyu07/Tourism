...
<?php if (!$app->isAdmin()) { ?>
	<?php foreach ($gallery as $count => $photo) { ?>
		<div class="img gallery-item">
			<img src="<?php echo $photo->thumbImageFilePath; ?>" alt="" />
			<?php if (!empty(preg_replace('/\s+/', '', $photo->captionTitle))) { ?>
				<span class="image-caption"><?php echo $photo->captionTitle; ?></span>
			<?php } ?>
			<?php if (!empty(preg_replace('/\s+/', '', $photo->captionDescription))) { ?>
				<span class="image-credits"><?php echo $photo->captionDescription; ?></span>
			<?php } ?>
		</div>
	<?php } ?>
<?php } else { ?>
