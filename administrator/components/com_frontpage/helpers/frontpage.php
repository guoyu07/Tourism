...
class FrontpageHelper
{
	...
	public static function addSubmenu($vName = 'items')
	{
		...
		JSubMenuHelper::addEntry(
			JText::_('Items'),
			'index.php?option=com_frontpage&view=items',
			($vName == 'items')
		);	
	} else {
	...
	if(version_compare(JVERSION,'3','<')){
		JSubMenuHelper::addEntry(
			JText::_(''),
			'index.php?option=com_frontpage&view=',
			($vName == '')
		);	
	} else {
		JHtmlSidebar::addEntry(
	...
	public static function getActions($categoryId = 0)
	{
		...
		if (empty($categoryId))
		{
			$assetName = 'com_frontpage';
			$level = 'component';
		}
		else
		{
			$assetName = 'com_frontpage.category.'.(int) $categoryId;
			$level = 'category';
		}
	
		$actions = JAccess::getActions('com_frontpage', $level);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_frontpage/elements/extensions.xml', 'JXMLElement');		        
	...
	public static function getCategoryViews()
	{
		...
		foreach($extensions as $extension ) {
			$views[$extension->listview->__toString()] = 'com_frontpage.'.$extension->name->__toString();
		}
		return $views;
	}	
}
...
abstract class JHtmlFrontpage
{
	...
	{
		$extensions = FrontpageHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_frontpage.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__frontpage_categories AS a');
