...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_frontpage&task=items.saveOrderAjax&tmpl=component';
  ...
  if (count($this->items)) : 
  		foreach ($this->items as $i => $item) :
    			...
    			if (JFactory::getApplication()->input->get('function', null)) {
    				$onclick= "onclick=\"window.parent.jSelectItem_id('".$item->id."', '".$this->escape($item->title)."', '','id')\" ";
    			}  	
    
 				$link = JRoute::_( 'index.php?option=com_frontpage&view=item&task=item.edit&id='. $item->id );
  		...
  		?>
				<tr class="row<?php echo $i % 2; ?>"">
					...
					<div class="pull-left">
								...
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->checked_out_by, $item->checked_out_time, 'items.', $canCheckin); ?>
							<?php endif; ?>
														<?php if ($canEdit) : ?>
						...
						<div class="pull-left">
							<?php
								...
								JHtml::_('dropdown.edit', $item->id, 'item.');
								...
								if ($archived) :
									JHtml::_('dropdown.unarchive', 'cb' . $i, 'items.');
								else :
									JHtml::_('dropdown.archive', 'cb' . $i, 'items.');
								endif;
