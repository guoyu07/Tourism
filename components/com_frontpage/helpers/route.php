...
/**
 * Frontpage Component Route Helper
 *
 * @static
 * @package		Frontpage
 * @subpackage	Helpers
 ...
 */
abstract class FrontpageHelperRoute
{
	protected static $lookup;
	/**
	 * @param	int	The route of the frontpage
	 */
	public static function getFrontpageRoute($id, $catid)
	{
		$needles = array(
			'items'  => array((int) $id)
		);
		//Create the link
		$link = 'index.php?option=com_frontpage&view=items&id='. $id;
		if ($catid > 1) {
			$categories = JCategories::getInstance('Items');
		...
		if ($item = FrontpageHelperRoute::_findItem($needles)) {
			$link .= '&Itemid='.$item;
		};
	...
	public static function getCategoryRoute($catid, $language = 0)
	{
		...
		$catviews = FrontpageHelper::getCategoryViews();
		...
		{
			//Create the link
			$link = 'index.php?option=com_frontpage&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null, $identifier = 'id')
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component	= JComponentHelper::getComponent('com_frontpage');
