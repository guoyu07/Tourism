<?php
/**
 * @version		$Id:router.php 1 2015-08-08Z  $
 * @package		Frontpage
 * @subpackage 	Router
...
require_once(JPATH_ADMINISTRATOR.'/components/com_frontpage/helpers/frontpage.php');

defined('_JEXEC') or die('Restricted access');

  function FrontpageBuildRoute( &$query )
  {
  	$segments = array();
	...
	$params = JComponentHelper::getParams('com_frontpage');
	...
	{
		$view = $query['view'];
		if (empty($query['Itemid']) || empty($menuItem) || $menuItem->component != 'com_frontpage')
		{
			$segments[] = $query['view'];
			unset($query['view']);
		}
	...
	if (isset($view) && in_array($view, $listviews))
	{
		...
		if ((isset($query['id']) && ($mId != (int) $query['id'])) || $mView != $view)
		{
			...
			$categories = JCategories::getInstance('Frontpage', $options);
	...
	return $segments;  	
  } // End FrontpageBuildRoute function
  
  function FrontpageParseRoute( $segments )
  {
  	$vars = array();
